...
for(i in 1:S)
{
  ...
  for (j in 1:B)
  {
    ...
    # DiM
    taudimCentered[i, j] = mean(Ytreat) - mean(Ycontrol) - CATE[i]
    ...
    taugobCentered[i, j] =  mean(yti-yci) - CATE[i]
    ...
    taucal2Centered[i, j] = mean(yti-yci) - CATE[i]
    ...
    taucal1Centered[i, j] = mean(fittedT3-fittedC3) - CATE[i]
...
varsName = paste("Poisson_CATE_VarRatios_N", N, ".csv", sep = '')
