...
for(i in 1:nsims)
{
  ...
  DiM[i] = mean(outcomes[Z]) - mean(outcomes[!Z])
  cat("Iteration", i, "\n")
}
...
cat("N-scaled variance of DiM\t", N*var(DiM), "\n")

save(uncalibrated, file = paste("wagerUncalibrated_SampleSplit_N", N, ".RData", sep=''))
save(calibrated, file = paste("wagerCalibrated_SampleSplit_N", N, ".RData", sep=''))
save(DiM, file = paste("wagerDiM_SampleSplit_N", N, ".RData", sep=''))

dtf = data.frame(uncal = N*var(uncalibrated), cal = N*var(calibrated), DiM = N*var(DiM))
write.csv(dtf, file = paste("wager_SampleSplit_N", N, ".csv", sep=''), row.names = F)
